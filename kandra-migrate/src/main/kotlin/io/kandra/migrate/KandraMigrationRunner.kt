...
private fun runClaimedDdlAction(session: CqlSession, lockName: String, holder: String, action: () -> Unit) {
    ...
    if (!markDdlLockDone(session, lockName, holder)) {
        throw KandraSchemaException(
            ...
            "DDL pass took longer than the ${DDL_BOOTSTRAP_STALE_THRESHOLD.seconds}s staleness " +
            "threshold. Verify the resulting schema in '$lockName' is consistent, and investigate why " +
            "the DDL pass (or cluster latency) exceeded the staleness threshold."
        )
    }
