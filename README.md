...
If `kandra-codegen` is on the classpath (with `koin-core`/`kodein-di` also present), prefer its
generated typed accessors over hand-typed qualifier strings — a typo or entity rename now fails to
*compile* instead of throwing at first resolution, and no unchecked cast is needed at the call site:

```kotlin
// generated by kandra-codegen — wraps the exact named("UserSuspendRepo") lookup below internally
fun KoinComponent.userRepo(): KandraSuspendRepository<User> = get(named("UserSuspendRepo"))
```

Without `kandra-codegen`, use the hand-typed lookup `kandraKoin()` binds under directly:
...
**Strict Mode (0.4.7)** — opt-in warning for a common multi-DC footgun: `LOCAL_ONE`/`ONE` reads/writes
hit a single node's local view, which may not yet have the latest write from another DC.

```kotlin
install(Kandra) {
    consistency {
        strictMode = true   // opt-in, default false — WARN-only, never throws
    }
    loadBalancing {
        allowedRemoteDcs = listOf("dc-eu")   // presence of remote DCs is what makes strictMode fire
    }
}
```

Logs a WARN whenever a query resolves to `LOCAL_ONE`/`ONE` **and** the deployment is multi-DC
...
- `SchemaMode.AUTO_CREATE`/`AUTO_MIGRATE`'s DDL, and `KandraMigrationRunner`'s own bookkeeping-table
  bootstrap, are now guarded by an LWT claim so only one of several concurrently-starting instances
...
> **No automatic retry on transient errors.** Every other write path retries on `WriteTimeoutException`/
> `ReadTimeoutException`/`NoNodeAvailableException` per `RetryConfig`; `update()`/`updateSuspend()` on a
> `@Version` entity deliberately execute the `IF version = ?` statement exactly once — on a transient
> error the write may have already been applied server-side, and retrying the identical conditional
> statement would then see `[applied] = false` and raise a **spurious** `KandraOptimisticLockException`.
> If you want retry-on-timeout semantics for a versioned update, catch the transient exception yourself,
> re-fetch the entity, and reissue `update()` with the freshly-read `old` — see
> [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md) for the full recipe and [ISS-032](docs/issues/ISS-032-versioned-update-spurious-optimistic-lock.md).
...
`gcGraceSeconds` on `@ScyllaTable` controls how long the tombstoned row's primary key survives before ScyllaDB's compaction reclaims it. For a `findActive()` that filters out soft-deleted rows, add `@SoftDelete(ttlSeconds = ..., markerProperty = "isDeleted")` with a `Boolean` field — see [`docs/features/repositories.md`](docs/features/repositories.md#findactive-soft-delete-entities-only) and [ISS-007](docs/issues/ISS-007-find-active-soft-delete.md). `@LookupIndex` rows on a soft-deleted entity are deliberately kept alive until the entity's own TTL expires, not hard-deleted — see [ISS-030](docs/issues/ISS-030-soft-delete-removes-lookup-rows.md) and [ISS-035](docs/issues/ISS-035-lookupindex-softdelete-storage-growth.md) for the storage-growth implication on high-churn tables combining both annotations.

> **`findActive(allowFullScan: Boolean = false)` — breaking change as of GH #12.** If the marker column
> has `@SecondaryIndex`, `findActive()` queries it directly, no `ALLOW FILTERING` needed. Without one,
> it now **throws `KandraQueryException`** unless called with `allowFullScan = true` — previously it
> silently ran `ALLOW FILTERING` with just a WARN log. Add `@SecondaryIndex` to the marker column, or
> pass `allowFullScan = true` to keep the old scatter-gather behavior. See
> [ISS-036](docs/issues/ISS-036-findactive-allow-filtering-scope.md).
